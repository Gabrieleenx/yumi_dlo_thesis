...
class SPR(object):
    ...
    def SPR_register(self, X, Y, opt):
        ...
        if opt.get('normalize'):
            print('normalize')
            [X, Y, normal] = CPD_utils_gpu.cpd_normalize(X, Y)

        if self.seq == 0:
            M, D = Y.shape
            Phi = LocalReg(np.transpose(Y), opt.get('knn'), D)
            self.Phi = cp.asarray(Phi)
            self.seq = 1
...
def cpd_MLLE(X, Y, beta, lambda0, max_it, tol, viz, outliers,
    ...
    G = CPD_utils_gpu.cpd_G(Y, Y, beta)
    ...
    while (iter < max_it) and (ntol > tol) and (sigma2 > 7.7e-4):
        ...
        dP1 = cp.sparse.spdiags(P1, diags=0, m=M, n=M).toarray()

        W = cp.linalg.solve(cp.dot(dP1, G) + (lambda_var * sigma2 * cp.eye(M)) + tau * sigma2 * cp.dot(Phi, G),
            (PX - cp.dot(dP1, Y) - tau * sigma2 * cp.dot(Phi, Y)))
        ...
        sigma2 = cp.abs((cp.sum(cp.multiply(pow(X, 2), cp.transpose(cp.tile(Pt1, (D,1)))))
                        + cp.sum(cp.multiply(pow(T, 2), cp.transpose(cp.tile(P1, (D,1)))))
                        - 2 * cp.trace(cp.dot(cp.transpose(PX), T))) / (Np * D))
...
def GlobalReg(X, T, sigma2, outliers):
    ...
    c = ((((2 * cp.pi * sigma2) ** D / 2) * (outliers / (1 - outliers))) * (M / N))
    P_den += c
